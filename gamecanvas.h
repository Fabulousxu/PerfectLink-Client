...
inline QPoint directionPoint(Direction d) {
  return d & 1 ? QPoint(d - 2, 0) : QPoint(0, d - 1);
}
inline Direction opposite(Direction d) {
  return (Direction)(d < Down ? d + 2 : d - 2);
}
inline QPoint neighbor(int x, int y, Direction d) {
  return d & 1 ? QPoint(x + d - 2, y) : QPoint(x, y + d - 1);
}
...
class Block : public Cell {
  Q_OBJECT
 public:
  Block(int x, int y, int pt,
        QWidget *parent); /* -1是墙, 0是地板, 1-32是图案 */
...
class Select : public Cell {
 ...
 public:
  ...
  Select(QPoint p, int pt, QWidget *parent)
      : Select(p.x(), p.y(), pt, parent) {}
  void paintEvent(QPaintEvent *event) override;
  void setPattern(int pt);
  static void loadPicture();
...
class Player : public Cell {
 ...
 public:
  ...
  Player(QPoint p, int pt, QWidget *parent)
      : Player(p.x(), p.y(), pt, parent) {}
...
class GameCanvas : public QWidget {
 ...
 public:
  ...
  void initializePlayer(quint64 id,
                        QPoint p); /* 用于游戏开始时设置玩家初始位置 */
