...
QVector<QPoint> Game::match(const QPoint &a, const QPoint &b) {
    QVector<QPoint> path;
    a == b || getBlock(a) != getBlock(b) || isFloor(getBlock(a)) || isWall(getBlock(a)) || isRemoving(getBlock(a)) ||
        !(path = matchLine(a, b)).empty() || !(path = matchTurn(a, b)).empty() || !(path = matchTurn2(a, b)).empty();
    return path;
}
...
QVector<QPoint> Game::matchLine(const QPoint &a, const QPoint &b) {
    ...
    if (a.x() == b.x()) {
        if (b.y() > a.y()) {
            for (auto y = a.y() + 1; y < b.y(); ++y) { if (!isFloor(block[a.x()][y])) { return path; } }
            for (auto y = a.y(); y <= b.y(); ++y) { path.append(QPoint(a.x(), y)); }
        } else {
            for (auto y = a.y() - 1; y > b.y(); --y) { if (!isFloor(block[a.x()][y])) { return path; } }
            for (auto y = a.y(); y >= b.y(); --y) { path.append(QPoint(a.x(), y)); }
        }
        ...
        if (b.x() > a.x()) {
            for (auto x = a.x() + 1; x < b.x(); ++x) { if (!isFloor(block[x][a.y()])) { return path; } }
            for (auto x = a.x(); x <= b.x(); ++x) { path.append(QPoint(x, a.y())); }
        } else {
            for (auto x = a.x() - 1; x > b.x(); --x) { if (!isFloor(block[x][a.y()])) { return path; } }
            for (auto x = a.x(); x >= b.x(); --x) { path.append(QPoint(x, a.y())); }
        }
...
QVector<QPoint> Game::matchTurn(const QPoint &a, const QPoint &b) {
    if (a.x() == b.x() || a.y() == b.y()) { return QVector<QPoint>(); }
    QVector<QPoint> path1, path2;
    QPoint turn1(a.x(), b.y()), turn2(b.x(), a.y());
    if (isFloor(getBlock(turn1)) && !(path1 = matchLine(a, turn1)).empty() && !(path2 = matchLine(turn1, b)).empty()) {
        path1.removeLast();
        return path1 + path2;
    }
    if (isFloor(getBlock(turn2)) && !(path1 = matchLine(a, turn2)).empty() && !(path2 = matchLine(turn2, b)).empty()) {
        path1.removeLast();
        return path1 + path2;
...
QVector<QPoint> Game::matchTurn2(const QPoint &a, const QPoint &b) {
    ...
    if (pos[0] == 1) {
        if (pos[1] > a.x()) {
            for (auto x = a.x(); x <= pos[1]; ++x) { path.append(QPoint(x, a.y())); }
        } else { for (auto x = a.x(); x >= pos[1]; --x) { path.append(QPoint(x, a.y())); } }
        if (b.y() > a.y()) {
            for (auto y = a.y() + 1; y < b.y(); ++y) { path.append(QPoint(pos[1], y)); }
        } else { for (auto y = a.y() - 1; y > b.y(); --y) { path.append(QPoint(pos[1], y)); } }
        if (b.x() > pos[1]) {
            for (auto x = pos[1]; x <= b.x(); ++x) { path.append(QPoint(x, b.y())); }
        } else { for (auto x = pos[1]; x >= b.x(); --x) { path.append(QPoint(x, b.y())); } }
    } else if (pos[0] == 2) {
        if (pos[1] > a.y()) {
            for (auto y = a.y(); y <= pos[1]; ++y) { path.append(QPoint(a.x(), y)); }
        } else { for (auto y = a.y(); y >= pos[1]; --y) { path.append(QPoint(a.x(), y)); } }
        if (b.x() > a.x()) {
            for (auto x = a.x() + 1; x < b.x(); ++x) { path.append(QPoint(x, pos[1])); }
        } else { for (auto x = a.x() - 1; x > b.x(); --x) { path.append(QPoint(x, pos[1])); } }
        if (b.y() > pos[1]) {
            for (auto y = pos[1]; y <= b.y(); ++y) { path.append(QPoint(b.x(), y)); }
        } else { for (auto y = pos[1]; y >= b.y(); --y) { path.append(QPoint(b.x(), y)); } }
    } return path;
}
