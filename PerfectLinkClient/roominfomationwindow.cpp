	...
	, ui(new Ui::RoomInfomationWindowClass())
{
	...
	ui->roomIdInput->setValidator(new QRegularExpressionValidator(QRegularExpression("[0-9]+$")));
	...
	for (auto pRoomDiaplay : roomDisplay) { pRoomDiaplay->hide(); }
	connect(errorShowTimer, &QTimer::timeout, this, [this] { ui->errorLabel->clear(); });
	connect(this, &RoomInfomationWindow::requireRoomRequest, this, [this](int pn) { showError("获取房间信息中..."); });
	connect(ui->modeButton, &QPushButton::clicked, this, &RoomInfomationWindow::onModeButton);
	connect(ui->backButton, &QPushButton::clicked, this, &RoomInfomationWindow::onBackButton);
	connect(ui->refreshButton, &QPushButton::clicked, this, &RoomInfomationWindow::onRefreshButton);
	connect(ui->roomIdInput, &QLineEdit::textChanged, this, &RoomInfomationWindow::onRoomIdInput);
	for (int i = 0; i < 5; ++i) {
		connect(enterRoomButton[i], &QPushButton::clicked, this, [this, i] { onEnterRoomButton(i); });
	}
...
void RoomInfomationWindow::onRequireRoomSuccess(const QVector<QPair<quint64, int>> &roomInfomation)
{
	showError("房间信息获取成功");
	int i;
...
{
	for (auto button : enterRoomButton) { button->setEnabled(true); }
...
{
	showError("进入房间中...");
	for (auto button : enterRoomButton) { button->setEnabled(false); }
