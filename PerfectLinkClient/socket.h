...
#else
#define SERVER_IP "10.119.11.142"
#endif
...
class Socket : public QTcpSocket {
 ...
 private:
 public slots:
  void onSignupRequest(const QString &nickname,
                       const QString &password); /* 响应注册请求 */
  void onLoginRequest(quint64 id, const QString &password); /* 响应登录请求 */
  void onLogoffRequest(quint64 id); /* 响应登出请求 */
  void onCreateRoomRequest(int playerNumber, int w, int h, int patternNumber,
                           int time);          /* 响应创建房间 */
 ...
 signals:
  ...
  void requireRoom(const QVector<QPair<quint64, int>>
                       &roomInfomation); /* 成功获取房间信息 */
  void enterRoomSuccess(int playerLimit, int width, int height,
                        int patternNumber, int time,
                        const QVector<QPair<quint64, QPair<QString, bool>>>
                            &playerInfomation); /* 成功进入房间 */
  ...
  void gameBegin(const QVector<QVector<int>> &map,
