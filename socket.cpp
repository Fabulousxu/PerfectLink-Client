...
void Socket::onCreateRoomRequest(int playerNumber, int w, int h,
    ...
    switch (reply) {
      case Reply::Signup: {
        ...
        if (flag) {
          ...
          emit signupFail(data.value("error").toString());
        }
      } break;
      case Reply::Login: {
        auto flag = data.value("state").toBool();
        if (flag) {
          ...
          emit loginFail(data.value("error").toString());
        }
      } break;
        // case Reply::Logoff: {
        //	auto flag = data.value("state").toBool();
      ...
      case Reply::EnterRoom: {
        ...
        if (flag) {
          ...
          for (auto jsonValue : data.value("playerInfo").toArray()) {
            ...
            playerInfomation.append(QPair<quint64, QPair<QString, bool>>(
                id, QPair<QString, bool>(nickname, prepare)));
          }
          emit enterRoomSuccess(playerLimit, width, height, patternNumber, time,
                                playerInfomation);
        } else {
      ...
      case Reply::ExitRoom: {
        ...
        if (flag) {
          ...
          emit exitRoomFail(data.value("error").toString());
        }
      } break;
      case Reply::Prepare: {
      ...
      case Reply::Begin: {
        ...
        for (auto xValue : data.value("map").toArray()) {
          QVector<int> xMap;
          for (auto yValue : xValue.toArray()) {
            xMap.append(yValue.toInt());
          }
          map.append(xMap);
        }
