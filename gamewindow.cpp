...
GameWindow::GameWindow(QWidget *parent)
    : QWidget(parent), ui(new Ui::GameWindowClass()), gameCanvas(nullptr) {
  ui->setupUi(this);
  setParent(parent);
  ...
  for (Direction d = Up; d <= Right; d = (Direction)(d + 1)) {
    ...
    connect(keyTimer[d], &QTimer::timeout, this,
            [this, d] { emit moveRequest(d); });
  }
  connect(errorShowTimer, &QTimer::timeout, this,
          [this] { ui->errorLabel->clear(); });
  connect(ui->exitRoomButton, &QPushButton::clicked, this,
          &GameWindow::onExitRoomButton);
  connect(ui->prepareButton, &QPushButton::clicked, this,
          &GameWindow::onPrepareButton);
  ...
  connect(countdownTimer, &QTimer::timeout, this, [this] {
    if (countdown > 0) {
      countdown -= 20;
      ui->timeInnerRect->resize(
          (ui->timeOuterRect->width() - 6) * countdown * 0.001 / time,
...
void GameWindow::setParameter(int playerLimit, int w, int h, int patternNumber,
                              int time) {
  mode = playerLimit - 1;
  width = w;
  height = h;
...
void GameWindow::keyPressEvent(QKeyEvent *event) {
  ...
  static Qt::Key key[4] = {Qt::Key_Up, Qt::Key_Left, Qt::Key_Down,
                           Qt::Key_Right};
  for (Direction d = Up; d <= Right; d = (Direction)(d + 1)) {
    if (event->key() == key[d]) {
      emit moveRequest(d);
      if (!keyStack.empty()) {
        keyTimer[keyStack.top()]->stop();
      }
      keyStack.push(d);
      keyTimer[d]->start();
      return;
...
void GameWindow::onEnterRoomSuccess(
  ...
  ui->prepareButton->setStyleSheet(
      "QPushButton {\n	color: rgb(255, 255, 255);\n	background-color: "
      "rgb(129, 86, 0);\n	border: 2px solid rgb(255, 255, 255);\n	"
      "border-radius: 15px;\n	padding: 20px 40px 20px "
      "40px;\n}\nQPushButton:hover {\n	background-color: rgb(216, 189, "
      "122);\n}\nQPushButton:pressed {\n	background-color: rgb(76, 51, "
      "0);\n}");
...
void GameWindow::onPrepare(quint64 id, bool flag, quint64 selfId) {
  if (id == selfId) {
    ...
    if (flag) {
      ...
      prepareLabel[id]->hide();
    }
  }
}

void GameWindow::onGameBegin(
    const QVector<QVector<int>> &map,
    const QVector<QPair<quint64, QPoint>> &playerPosition) {
  if (gameCanvas) {
...
void GameWindow::onGameEnd(const QVector<QPair<quint64, int>> &rank,
                           quint64 id) {
  delete gameCanvas;
  gameCanvas = nullptr;
  countdownTimer->stop();
...
void GameWindow::onExitRoomButton() { emit exitRoomRequest(); }

void GameWindow::onPrepareButton() { emit prepareRequest(!isPrepare); }
...
void GameWindow::onExitRoomFail(const QString &error) { showError(error); }

void GameWindow::onCreateRoomSuccess(quint64 rid, quint64 id,
