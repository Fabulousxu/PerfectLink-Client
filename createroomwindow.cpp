...
CreateRoomWindow::CreateRoomWindow(QWidget *parent)
    : QWidget(parent), ui(new Ui::CreateRoomWindowClass()) {
  ui->setupUi(this);
  setParent(parent);
  ...
  ui->widthInput->setValidator(
      new QRegularExpressionValidator(QRegularExpression("[0-9]+$")));
  ui->heightInput->setValidator(
      new QRegularExpressionValidator(QRegularExpression("[0-9]+$")));
  ui->patternNumberInput->setValidator(
      new QRegularExpressionValidator(QRegularExpression("[0-9]+$")));
  ...
  connect(ui->modeButton, &QPushButton::clicked, this,
          &CreateRoomWindow::onModeButton);
  connect(ui->backButton, &QPushButton::clicked, this,
          &CreateRoomWindow::onBackButton);
  connect(ui->levelButton, &QPushButton::clicked, this,
          &CreateRoomWindow::onLevelButton);
  connect(ui->heightInput, &QLineEdit::editingFinished, this,
          &CreateRoomWindow::onHeightInput);
  connect(ui->widthInput, &QLineEdit::editingFinished, this,
          &CreateRoomWindow::onWidthInput);
  connect(ui->patternNumberInput, &QLineEdit::editingFinished, this,
          &CreateRoomWindow::onPatternNumberInput);
  connect(ui->timeInput, &QLineEdit::editingFinished, this,
          &CreateRoomWindow::onTimeInput);
  connect(ui->createRoomButton, &QPushButton::clicked, this,
          &CreateRoomWindow::onCreateRoomButton);
}
...
void CreateRoomWindow::onModeButton() { setMode(mode < 3 ? mode + 1 : 1); }
...
void CreateRoomWindow::onLevelButton() { setLevel(level < 2 ? level + 1 : 0); }
...
void CreateRoomWindow::onHeightInput() {
  ...
  if (h.isEmpty()) {
    ui->heightInput->setText(QString::number(height));
  } else if (h.toInt() != height && setHeight(h.toInt())) {
    setLevel(3);
  }
  setFocus();
}
...
void CreateRoomWindow::onWidthInput() {
  ...
  if (w.isEmpty()) {
    ui->widthInput->setText(QString::number(width));
  } else if (w.toInt() != width && setWidth(w.toInt())) {
    setLevel(3);
  }
  setFocus();
}
...
void CreateRoomWindow::onPatternNumberInput() {
  ...
  if (p.isEmpty()) {
    ui->patternNumberInput->setText(QString::number(patternNumber));
  } else if (p.toInt() != patternNumber && setPatternNumber(p.toInt())) {
    setLevel(3);
  }
  setFocus();
}
...
void CreateRoomWindow::onTimeInput() {
  ...
  if (t.isEmpty()) {
    ui->timeInput->setText(QString::number(time));
  } else if (t.toInt() != time && setTime(t.toInt())) {
    setLevel(3);
  }
  setFocus();
}
